...
function verifyTelegramInitData(initData) {
  ...
  try {
    ...
    if (providedHash.length !== computedHash.length) return false;

    const authDate = Number(params.get('auth_date') || 0);
    const now = Math.floor(Date.now() / 1000);
    if (!authDate || Math.abs(now - authDate) > 86400) return false;

    return crypto.timingSafeEqual(computedHash, providedHash);
  } catch (e) {
...
app.post('/api/auth', (req, res) => {
  ...
  if (!tgUser || !tgUser.id) {
    return res.status(400).json({ error: 'No Telegram user data found' });
  }
...
// ─── START ────────────────────────────────────────────────────────
app.listen(PORT, () => {
  console.log(`Lumina Mini App running on port ${PORT}`);
});
